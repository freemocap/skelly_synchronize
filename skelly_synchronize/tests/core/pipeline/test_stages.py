...
def test_brightness_lag_stage_normalizes_lags(monkeypatch, tmp_path):
    ...
    # encode the camera's raw lag in the "series" so the fake event-finder
    # can decode it back out, without needing to track call order.
    monkeypatch.setattr(
        stages_module,
        "compute_brightness_series",
        lambda video: np.array([raw_lag_seconds[video.camera_name]]),
    )
    monkeypatch.setattr(
        stages_module,
    ...
    assert lag_by_camera["cam_a"] == max_lag - raw_lag_seconds["cam_a"]
    assert lag_by_camera["cam_b"] == max_lag - raw_lag_seconds["cam_b"]
...
class FakeBackend:
    ...
    def trim(self, filepath, start_seconds, end_seconds, output_path):
        camera_name = Path(filepath).stem
        if camera_name in self.should_fail_for:
            raise RuntimeError(f"boom for {camera_name}")
...
def test_trim_video_worker_uses_ffmpeg_backend_for_final_probe(monkeypatch, tmp_path):
    ...
    lag = LagResult(camera_name="raw_cam_a", lag_seconds=1.0)

    result = _trim_video_worker(
        video_info, lag, VideoBackendKind.FFMPEG, tmp_path, minimum_duration=5.0
    )

    assert result.camera_name == "synced_cam_a"


def test_trim_stage_isolates_per_camera_errors(monkeypatch, tmp_path):
    ...
    lags = [
        LagResult(camera_name="cam_a", lag_seconds=0.0),
        LagResult(camera_name="cam_b", lag_seconds=0.0),
    ]
...
def test_trim_stage_succeeds_when_all_cameras_trim_cleanly(monkeypatch, tmp_path):
    monkeypatch.setattr(
        stages_module, "ProcessPoolExecutor", concurrent.futures.ThreadPoolExecutor
    )
    ...
    lags = [
        LagResult(camera_name="cam_a", lag_seconds=0.0),
        LagResult(camera_name="cam_b", lag_seconds=1.0),
    ]
    context = PipelineContext(
        request=request, videos=videos, lags=lags, synchronized_folder_path=output_dir
    )
    ...
    assert {video.camera_name for video in result.videos} == {
        "synced_cam_a",
        "synced_cam_b",
    }
...
def test_verify_synchronized_framerate_stage_raises_on_any_mismatch(tmp_path):
    ...
    # fps discrepancy between cameras (e.g. from a lossy backend re-encode)
...
def test_verify_synchronized_frame_count_stage_raises_on_any_mismatch(tmp_path):
    ...
    # fps doesn't help if one camera's output is a frame longer or shorter.
    request = _make_request(tmp_path)
    videos = [
        _make_video_info("synced_cam_a", frame_count=300),
