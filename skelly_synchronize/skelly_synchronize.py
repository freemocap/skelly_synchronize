...
def synchronize(
    ...
    synchronized_folder_path = create_directory(
    ...
    audio_signal_dict = get_audio_files(
        video_info_dict=video_info_dict, audio_extension="wav", audio_folder_path=audio_folder_path
    )
    ...
    trim_videos(
        ...
        synchronized_folder_path=synchronized_folder_path,
        lag_dict=lag_dict,
        fps=fps,
...
def get_audio_files(video_info_dict: Dict[str, dict], audio_extension: str, audio_folder_path: Path) -> dict:
...
def trim_videos(
    ...
    for video_dict in video_info_dict.values():
        ...
        synced_video_name = _name_synced_video(
...
def synchronize_videos(raw_video_folder_path: Path, file_type: str = ".mp4"):
    ...
    synchronized_video_folder_path = synchronize(
